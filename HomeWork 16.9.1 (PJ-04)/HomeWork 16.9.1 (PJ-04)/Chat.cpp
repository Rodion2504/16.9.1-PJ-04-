...
{
	std::cout << "Пожалуйста, введите логин для регистрации: ";
...
auto Chat::verification() //authorization
{
	...
	std::cout << "Ошибка: логин/пароль недействителен. \n";
	return false;
}
...
void Chat::view_messages()
{
	for (auto element : _boxMessages)
	{
		...
		{
			std::cout << "Ответить на личное сообщение? \n 0: НЕТ \n 1: ДА \n";
			bool answer{ false };
			std::cin >> answer;
			if (answer)
			{
...
void Chat::strart_menu()
{
	...
	do
	{
		...
		case '0':
			offChat = false;
			std::cout << "Выход из программы, увидимся позже! \n";
			break;
...
void Chat::working_session()
{
	...
	{
		std::cout << "Ошибка авторизации, свяжитесь с разработчиком \n";
		return;
	}

	std::cout << "Авторизован: " << _currentUser->get_login() << "\n";
	...
	do
	{
		std::cout
			...
			<< counter_new_message()
			<< ") Сообщения \n"
			<< "2 Показать ВСЕ сообщения (история) \n"
			<< "3 Новые сообщения \n"
			<< "4 Новые личные сообщения \n"
